...
function Completed() {
  ...
  return (
    <div className='m-4 rounded-xl'>
      <h1 className='pb-4 text-2xl font-semibold text-red-300'>Showing Completed Orders:</h1>
      {orders === null ? ( // Check if orders is null
        ...
        orders.map(order => {
          ...
          return (
            <div key={order.id} className='p-4 bg-red-200 rounded-xl flex flex-col gap-4'>
              <p className='font-bold text-xl px-2'>{order.id}</p>
              ...
              <div className='bg-red-400 w-fit mx-auto px-2 rounded-lg'>{order.state}</div>
            </div>
          );
        })
      )}
